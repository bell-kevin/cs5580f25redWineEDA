...
\begin{frame}{Interaction Model Insights}
  \small
  \begin{itemize}
    \item Linear model: quality $=5.68 - 1.03\,\text{(vol. acidity)} + 1.20\,\text{(sulphates)} - 0.86\,\text{(vol. acidity} \times \text{sulphates)}$.
    \item Model explains \num{17.7}\% of variance; negative interaction captures diminishing sulphate returns.
    \item High sulphates help only when volatile acidity is under control; guides winemaking trade-offs.
  \end{itemize}
\end{frame}
...
\begin{frame}{Chemistry Clusters}
  ...
  \begin{tabular}{lS[table-format=4.0]S[table-format=1.2]S[table-format=2.2]S[table-format=1.3]S[table-format=1.3]}
    \toprule
    \textbf{Cluster} & {Size} & {Mean quality} & {Mean alcohol (\%)} & {Mean vol. acidity} & {Mean sulphates} \\
    \midrule
...
\begin{frame}{Synthesis}
  \begin{itemize}
    ...
    \item Interaction model (\emph{R}\textsuperscript{2}=0.177) shows sulphates lose impact when volatile acidity spikes.
